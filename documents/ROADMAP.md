...
- ✅ **Stufe 3b: Tone-of-Voice-Agent (manuell) nach der Umformulierung** — fester Agent `tone_of_voice` („Tonalitäts-Redakteur (Brand Voice)“). Ablauf: erst **Umformulieren** (EmpCo-konform), dann optional Button **„Tonalität anpassen“** je Finding — schleift den Vorschlag (inkl. gerade editiertem Textarea-Inhalt als Basis) auf die Brand Voice, **ohne** neue Umweltaussagen. Am Vorschlag wird transparent angezeigt, welche Redakteure beteiligt waren (`reformulations.agents_used`, z. B. „EmpCo-Redakteur + Tonalität (Brand Voice)“). Button erscheint nur, wenn der ToV-Agent aktiv ist; Prompt im Admin über „KI-Redakteure“ pflegbar. Brand Voice aktuell Platzhalter.
